...
class SafelinkBypassV2:
    ...
    def __init__(self, sb, original_ip=None, expected_proxy_ip=None, groq_manager=None, profile_id=None):
        ...
        self.main_domains = ["sfl.gl", "tutwuri.id", "downloadku.com"]
        self.cf_cookies = []
    ...
    def is_main_domain(self, url):
        """Cek apakah URL termasuk domain utama"""
        if not url:
            return False
        return any(domain in url.lower() for domain in self.main_domains)
    ...
    def handle_popups(self):
        """Handle popup - tutup yang bukan domain utama (FIXED: Timeout + Force Close + Max Limit)"""
        if not self.is_valid_session():
            return
        ...
        try:
            ...
            # Cek jika anchor hilang
            if self.anchor and self.anchor not in current_handles:
                log("CRITICAL: Anchor tab closed unexpectedly!", "ERROR")
                for h in current_handles:
                    try:
                        self.sb.driver.switch_to.window(h)
                        if self.is_main_domain(self.sb.get_current_url()):
                            self.anchor = h
                            log(f"Recovered anchor to new handle: {h}", "SUCCESS")
                            break
                    except:
                        continue
    ...
    def force_close_tab(self, handle):
        """Force close tab yang stuck/error"""
        try:
    ...
    def find_button(self):
        """Find target button dengan selector luas"""
        if not self.is_valid_session():
            return None
        
        selectors = [
            "//span[contains(text(), 'OPEN LINK')]/..",
            "button:contains('OPEN LINK')",
            "a:contains('OPEN LINK')",
            "button.bg-[#1A56DB]",
            "//button[contains(@class, 'btn') and contains(., 'Open')]",
            "//a[contains(@class, 'btn') and contains(., 'Open')]",
        ...
        for sel in selectors:
            try:
                if self.sb.is_element_visible(sel):
                    log(f"Found candidate: {sel[:40]}...", "SUCCESS")
                    return sel
            except:
                continue
        return None
            ...
            if selector:
                ...
                try:
                    ...
                    if self.sb.is_element_visible(selector):
                        log(f"Clicking button: {selector[:40]}...", "ACTION")
                ...
                log("No button, trying fallbacks...", "INFO")
                fallbacks = ["//span[contains(text(), 'OPEN LINK')]/..", "a:contains('Open')", "a:contains('Link')"]
                for fb in fallbacks:
                    try:
