...
class SafelinkBypassV2:
    ...
    def __init__(self, sb, original_ip=None, expected_proxy_ip=None, groq_manager=None, profile_id=None):
        ...
        self.main_domains = ["sfl.gl", "tutwuri.id", "downloadku.com", "cararegistrasi.com"]
        self.cf_cookies = []
    ...
    def is_valid_session(self):
        """Cek apakah sesi browser masih aktif - FIXED: Timeout protection"""
        try:
            ...
            try:
                ...
                if handles:
                    # Set timeout pendek untuk avoid stuck
                    try:
                        self.sb.driver.set_page_load_timeout(0.5)
                    except:
                        pass
    ...
    def back_to_anchor(self):
        """Kembali ke tab anchor (FIXED: Ultra-fast dengan timeout protection)"""
        if not self.is_valid_session():
            return
        try:
            # FIX: Set timeout sangat pendek untuk avoid stuck
            self.sb.driver.set_page_load_timeout(0.5)
            ...
            if self.anchor and self.sb.driver.current_window_handle != self.anchor:
                try:
                    ...
                    # Ignore timeout - yang penting switch berhasil
                    pass
    ...
    def handle_popups(self):
        """Handle popup - ULTRA AGGRESSIVE MODE V2: Tutup SEMUA tab kecuali anchor!
        
        STRATEGY BARU:
        1. Deteksi SEMUA tab
        2. TUTUP SEMUA kecuali anchor - NO MERCY!
        3. FORCE CLOSE dengan driver.close() langsung
        4. ALWAYS back to anchor after close
        """
        if not self.is_valid_session():
            return
        ...
        try:
            ...
            # Jika cuma 1 tab, tidak perlu close apapun
            ...
            if not self.anchor or self.anchor not in current_handles:
                ...
                # Try to recover anchor - cari tab dengan main domain
            ...
            # Find ALL tabs yang bukan anchor - TUTUP SEMUA!
            tabs_to_close = [h for h in current_handles if h != self.anchor]
            
            if not tabs_to_close:
                return  # No popups
            
            log(f"[POPUP] Detected {len(tabs_to_close)} popup tab(s) - FORCE CLOSING ALL!", "ACTION")
            
            # ULTRA AGGRESSIVE V2: TUTUP SEMUA dengan FORCE!
            closed_count = 0
            for h in tabs_to_close:
                # CRITICAL: Triple check this is not anchor
                if h == self.anchor:
                    log(f"[POPUP] SKIP: This is anchor tab!", "INFO")
                    continue
                
                # FORCE CLOSE - Langsung switch dan close, no mercy!
                try:
                    # Set timeout sangat pendek
                    try:
                        self.sb.driver.set_page_load_timeout(0.3)
                    except:
                        pass
                    
                    # Switch ke tab (ignore timeout)
                    try:
                        self.sb.driver.switch_to.window(h)
                    except:
                        pass  # Ignore timeout
                    
                    # FORCE CLOSE dengan driver.close() - PASTI TUTUP!
                    try:
                        self.sb.driver.close()
                        closed_count += 1
                        log(f"[POPUP] ✓ FORCE CLOSED tab {h[:8]}...", "SUCCESS")
                    except Exception as close_err:
                        # Jika driver.close() gagal, coba JavaScript
                        try:
                            self.sb.driver.execute_script("window.close();")
                            closed_count += 1
                            log(f"[POPUP] ✓ JS CLOSED tab {h[:8]}...", "SUCCESS")
                        except:
                            log(f"[POPUP] Tab {h[:8]}... already closed", "INFO")
                    
                except Exception as e:
                    log(f"[POPUP] Error closing tab: {str(e)[:30]}", "ERROR")
            ...
            if closed_count > 0:
                log(f"[POPUP] ✓✓✓ FORCE CLOSED {closed_count} popup(s)!", "SUCCESS")
            
            # CRITICAL: Always back to anchor
            self.back_to_anchor()
    ...
    def force_close_tab(self, handle):
        """Force close tab yang stuck/error - FIXED: Close TANPA tunggu load!"""
        ...
        try:
            ...
            # INSTANT CLOSE - Set timeout sangat pendek untuk avoid stuck
            try:
                # Set page load timeout sangat pendek
                self.sb.driver.set_page_load_timeout(0.5)
                
                try:
                    # Switch to the tab we want to close (dengan timeout protection)
                    self.sb.driver.switch_to.window(handle)
                except:
                    pass  # Ignore timeout saat switch
                
                # Close it - coba JavaScript dulu (paling cepat)
                try:
                    self.sb.driver.execute_script("window.close();")
                except:
                    # Fallback ke driver.close()
                    self.sb.driver.close()
                ...
                log(f"Error closing tab: {str(close_err)[:30]}", "ERROR")
    ...
    def find_button(self):
        ...
        - Step 6: Ada button <button> tanpa window.open dengan text "OPEN LINK"
          → Ini button ASLI yang menuju link final
    ...
    def run(self):
        ...
        while running and step < MAX_STEPS_HARD:
            ...
            # CRITICAL: Check for popups at start of each step (cleanup any leftover popups)
            try:
                current_handles = self.sb.driver.window_handles
                if len(current_handles) > 1:
                    log(f"[POPUP-CHECK] Found {len(current_handles)} tabs - cleaning up...", "INFO")
                    self.handle_popups()
            except:
                pass
            
            # EXTRA: Force popup check setiap 3 steps
            if step % 3 == 0:
                try:
                    current_handles = self.sb.driver.window_handles
                    if len(current_handles) > 1:
                        log(f"[POPUP-PERIODIC] Periodic check - {len(current_handles)} tabs found", "INFO")
                        self.handle_popups()
                except:
                    pass
            ...
            # EXTRA: Popup check setelah delay (popup bisa muncul kapan saja)
            try:
                current_handles = self.sb.driver.window_handles
                if len(current_handles) > 1:
                    log(f"[POPUP-AFTER-DELAY] Found {len(current_handles)} tabs", "INFO")
                    self.handle_popups()
            except:
                pass
            ...
            if selector:
                ...
                try:
                    ...
                    if self.sb.is_element_visible(selector):
                        ...
                        # INSTANT POPUP HANDLING - langsung cek dan tutup popup tanpa delay!
                        time.sleep(0.3)  # Minimal delay untuk popup muncul
                        tabs_after = len(self.sb.driver.window_handles)
                        
                        if tabs_after > tabs_before:
                            log(f"[POPUP] Detected {tabs_after - tabs_before} new tab(s) - CLOSING INSTANTLY!", "ACTION")
                            self.handle_popups()  # Close popup tabs IMMEDIATELY
                            log("[POPUP] ✓ Closed instantly", "SUCCESS")
                        
                        # Cek lagi setelah 0.5 detik (untuk popup yang lambat muncul)
                        time.sleep(0.5)
                        tabs_after2 = len(self.sb.driver.window_handles)
                        if tabs_after2 > tabs_before:
                            log(f"[POPUP] Detected late popup - CLOSING!", "ACTION")
                            self.handle_popups()
                        
                        # Simple delay setelah klik
                        self.human.random_delay(2.0, 4.0)
                        
                        # EXTRA: Popup check setelah delay (popup bisa muncul delayed)
                        try:
                            current_handles = self.sb.driver.window_handles
                            if len(current_handles) > 1:
                                log(f"[POPUP-POST-CLICK] Found {len(current_handles)} tabs after delay", "INFO")
                                self.handle_popups()
                        except:
                            pass
...
def main():
    ...
    log(f"\n[*] Loading links untuk Bot {BOT_NUMBER}...", "INFO")
    BOT_LINKS = load_bot_links(firebase_config, BOT_NUMBER)
    
    if not BOT_LINKS:
        log(f"ERROR: Tidak ada links untuk Bot {BOT_NUMBER}!", "ERROR")
        log(f"Tambahkan links ke Firebase: /bot{BOT_NUMBER}/links", "INFO")
        return
    
    log(f"[+] Loaded {len(BOT_LINKS)} links untuk Bot {BOT_NUMBER}", "SUCCESS")
    
    # Shuffle links agar random setiap run
    random.shuffle(BOT_LINKS)
    log(f"[+] Links di-shuffle untuk random selection setiap run", "INFO")
    
    for i, link in enumerate(BOT_LINKS, 1):
        log(f"    {i}. {link[:50]}...", "INFO")
    ...
    def start_cloudflared_tunnel(domain, local_port=9999, cloudflared_path='cloudflared', max_retries=3):
        """Start cloudflared tunnel in background - IMPROVED: Better validation & retry
        ...
        for retry in range(1, max_retries + 1):
            try:
                log(f"[CLOUDFLARED] Starting tunnel (attempt {retry}/{max_retries}): {domain} -> localhost:{local_port}", "ACTION")
                ...
                # Wait longer for tunnel to establish (cloudflared butuh waktu)
                log("[CLOUDFLARED] Waiting for tunnel to establish...", "INFO")
                time.sleep(5)
                
                # Check if process is still running
                if process.poll() is None:
                    log(f"[CLOUDFLARED] Tunnel process started (PID: {process.pid})", "SUCCESS")
                    
                    # CRITICAL: Verify tunnel actually works!
                    log("[CLOUDFLARED] Verifying tunnel connectivity...", "ACTION")
                    time.sleep(2)  # Extra wait untuk tunnel benar-benar ready
                    
                    # Test if we can connect through the tunnel
                    try:
                        ...
                        if result == 0:
                            log(f"[CLOUDFLARED] Tunnel is READY and accepting connections!", "SUCCESS")
                            return process, True
                        else:
                            log(f"[CLOUDFLARED] Tunnel not accepting connections yet (attempt {retry})", "ERROR")
                            process.terminate()
                            if retry < max_retries:
                                time.sleep(3)
                                continue
                    except Exception as e:
                        log(f"[CLOUDFLARED] Tunnel verification failed: {str(e)[:50]}", "ERROR")
                        process.terminate()
                        if retry < max_retries:
                            time.sleep(3)
                            continue
                    ...
                    try:
                        ...
                        log(f"[CLOUDFLARED] Tunnel failed: {stderr[:200]}", "ERROR")
                    except:
                        log(f"[CLOUDFLARED] Tunnel process died immediately", "ERROR")
                ...
                log(f"[CLOUDFLARED] Error starting tunnel (attempt {retry}): {str(e)[:100]}", "ERROR")
                if retry < max_retries:
                    time.sleep(3)
                    continue
                return None, False
        
        log("[CLOUDFLARED] All tunnel start attempts failed", "ERROR")
        return None, False
    ...
    def test_cloudflared_proxy(local_port=9999, max_retries=3):
        """Test if cloudflared proxy is working - IMPROVED: Multiple retries & better validation
        ...
        for retry in range(1, max_retries + 1):
            try:
                ...
                log(f"[CLOUDFLARED] Testing proxy (attempt {retry}/{max_retries})...", "ACTION")
                
                # Test 1: Get IP (dengan timeout lebih lama)
                test_ip = get_my_ip(proxy_dict)
                if not test_ip:
                    log(f"[CLOUDFLARED] Failed to get IP (attempt {retry})", "ERROR")
                    if retry < max_retries:
                ...
                # Test 2: HTTPS connectivity (dengan retry)
                https_ok = False
                for https_retry in range(2):
                    if test_proxy_https_basic(proxy_dict):
                        https_ok = True
                        break
                    if https_retry < 1:
                        time.sleep(1)
                ...
                if not https_ok:
                    log(f"[CLOUDFLARED] HTTPS test failed (attempt {retry})", "ERROR")
                    if retry < max_retries:
                        time.sleep(2)
                        continue
                ...
                # Test 3: Verify bisa akses web real (CRITICAL!)
                log("[CLOUDFLARED] Testing real web access...", "ACTION")
                test_urls = [
                ...
                for test_url in test_urls:
                    try:
                        ...
                        if resp.status_code == 200:
                            web_access_ok = True
                            log(f"[CLOUDFLARED] Web access OK: {test_url}", "SUCCESS")
                            break
                    except Exception as e:
                        log(f"[CLOUDFLARED] Web access failed: {test_url} - {str(e)[:50]}", "ERROR")
                        continue
                
                if not web_access_ok:
                    log(f"[CLOUDFLARED] Cannot access real websites (attempt {retry})", "ERROR")
                    if retry < max_retries:
                        time.sleep(3)
                        continue
                    return False, test_ip
                ...
                # All tests passed!
                log("[CLOUDFLARED] All tests passed! Proxy is READY", "SUCCESS")
                return True, test_ip
                
            except Exception as e:
                log(f"[CLOUDFLARED] Test error (attempt {retry}): {str(e)[:100]}", "ERROR")
                if retry < max_retries:
                    time.sleep(2)
                    continue
                return False, None
    ...
    def get_my_ip(proxy=None, max_retries=3):
        """Get current public IP with multiple fallback URLs - IMPROVED: Better retry logic"""
        ...
        timeout = 20 if proxy else 10  # Timeout lebih lama untuk proxy
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        try:
                            ...
                            # Bukan JSON, coba parse text (ifconfig.me)
                            ip = resp.text.strip()
                            if ip and len(ip) < 50:  # Validasi IP tidak terlalu panjang
                                return ip
                        
                except requests.exceptions.ProxyError as e:
                    if proxy:
                        log(f"Proxy connection error: {str(e)[:80]}", "ERROR")
                    continue
                except requests.exceptions.Timeout:
                    if proxy:
                        log(f"Proxy timeout untuk {url} (retry {retry+1}/{max_retries})", "ERROR")
                    continue
                except Exception as e:
                    if proxy:
                        log(f"get_my_ip error: {str(e)[:80]}", "ERROR")
                    continue
            
            # Jika semua URL gagal, tunggu sebentar sebelum retry
    ...
    def test_proxy_https_basic(proxy_dict, max_retries=2):
        """Verify proxy HTTPS connectivity - IMPROVED: Better retry & validation"""
        test_urls = [
        ...
        for retry in range(max_retries):
            for url in test_urls:
                try:
                    resp = requests.get(url, proxies=proxy_dict, timeout=15,
                                      headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/131.0.0.0"},
                                      allow_redirects=True)
                    if resp.status_code in [200, 301, 302]:
                        log(f"[HTTPS-TEST] OK: {url}", "SUCCESS")
                        return True
                except Exception as e:
                    log(f"[HTTPS-TEST] Failed: {url} - {str(e)[:50]}", "ERROR")
                    continue
    ...
    # Browser config (BACK TO INCOGNITO - user request)
    config = {
        "uc": True,
        "incognito": True,  # BACK TO INCOGNITO MODE
        "headless": False,
    ...
    log(f"[CONFIG] Browser: Incognito mode, UC stealth enabled", "INFO")
    ...
    def run_single_session(target_link):
        """Run satu session dengan link tertentu"""
        MAX_RETRIES = 1  # FIXED: 1x saja, tidak perlu 3x (buang waktu)
        
        for attempt in range(1, MAX_RETRIES + 1):
            log("\n" + "=" * 60, "INFO")
            log(f"STARTING BOT (Attempt {attempt}/{MAX_RETRIES})", "STEP")
            log(f"Target: {target_link}", "URL")
            log("=" * 60 + "\n", "INFO")
            
            # === CRITICAL: VALIDATE PROXY BEFORE BROWSER LAUNCH ===
            if VALID_PROXY:
                log("[PROXY-CHECK] Validating proxy before browser launch...", "ACTION")
                ...
                if CLOUDFLARED_PROXY_VALID:
                    ...
                    test_ip = get_my_ip(proxy_dict)
                    if not test_ip:
                        log("[PROXY-CHECK] Cloudflared proxy DEAD! Cannot get IP", "ERROR")
                        log("[PROXY-CHECK] Skipping this session, proxy tidak bisa dipakai", "ERROR")
                        return False
                    
                    # Quick web access test
                    try:
                        resp = requests.get("https://www.google.com", proxies=proxy_dict, timeout=10,
                                          headers={"User-Agent": "Mozilla/5.0"})
                        if resp.status_code != 200:
                            log("[PROXY-CHECK] Cloudflared proxy cannot access web!", "ERROR")
                            return False
                    except Exception as e:
                        log(f"[PROXY-CHECK] Cloudflared proxy FAILED: {str(e)[:50]}", "ERROR")
                        log("[PROXY-CHECK] Skipping this session", "ERROR")
                        return False
                    
                    log(f"[PROXY-CHECK] Cloudflared proxy OK! IP: {test_ip}", "SUCCESS")
                else:
                    # Test global proxy
                    proxy_url, _, _, _, _ = parse_proxy(VALID_PROXY)
                    proxy_dict = {"http": proxy_url, "https": proxy_url}
                    ...
                    test_ip = get_my_ip(proxy_dict)
                    if not test_ip:
                        log(f"[PROXY-CHECK] Proxy DEAD! Cannot get IP: {VALID_PROXY}", "ERROR")
                        log("[PROXY-CHECK] Skipping this session", "ERROR")
                        return False
                    ...
                    try:
                        resp = requests.get("https://www.google.com", proxies=proxy_dict, timeout=10,
                                          headers={"User-Agent": "Mozilla/5.0"})
                        if resp.status_code != 200:
                            log("[PROXY-CHECK] Proxy cannot access web!", "ERROR")
                            return False
                    except Exception as e:
                        log(f"[PROXY-CHECK] Proxy FAILED: {str(e)[:50]}", "ERROR")
                        log("[PROXY-CHECK] Skipping this session", "ERROR")
                        return False
                    
                    log(f"[PROXY-CHECK] Proxy OK! IP: {test_ip}", "SUCCESS")
            ...
            try:
                ...
                with SB(**config) as sb:
                    ...
                    # Override TARGET_URL dengan link yang dipilih
                    global TARGET_URL
    ...
    try:
        ...
        while True:
            ...
            # Pilih link random
            current_link = random.choice(BOT_LINKS)
            
            log(f"Link: {current_link[:60]}...", "INFO")
            log(f"Stats: {success_count} success, {fail_count} fail", "INFO")
            
            # Run session
            session_success = run_single_session(current_link)
