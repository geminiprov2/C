...
class SafelinkBypassV2:
    ...
    def handle_popups(self):
        """Handle popup - tutup yang bukan domain utama (FIXED: Timeout + Force Close + Max Limit + Never close anchor)"""
        if not self.is_valid_session():
            return
        ...
        try:
            ...
            if not self.anchor or self.anchor not in current_handles:
                log("CRITICAL: Anchor tab missing or invalid!", "ERROR")
                ...
                for h in current_handles:
                    try:
                        ...
                        if self.is_main_domain(url):
                            self.anchor = h
                            log(f"Recovered anchor to handle: {h[:8]}...", "SUCCESS")
                            break
                    except:
                        continue
                ...
                if not self.anchor and current_handles:
                    ...
                    log(f"Set anchor to first handle: {self.anchor[:8]}...", "INFO")
            ...
            if new_tabs:
                # FIX: Max 10 popup per cycle (prevent infinite loop)
                max_popups = 10
                if len(new_tabs) > max_popups:
                    log(f"Too many popups ({len(new_tabs)}), limiting to {max_popups}", "WARNING")
                    new_tabs = new_tabs[:max_popups]
                
                log(f"Detected {len(new_tabs)} new tab(s)", "ACTION")
                
                for h in new_tabs:
                    # CRITICAL: Double check this is not anchor
                    if h == self.anchor:
                        log(f"Skipping anchor tab in popup handler", "INFO")
                        self.processed.add(h)
                        continue
                    
                    # FIX: Max 5 detik per tab (total timeout)
                    tab_start = time.time()
                    max_tab_time = 5
                    ...
                    try:
                        ...
                        # Switch dengan timeout protection
                        try:
                            self.sb.driver.switch_to.window(h)
                            self.processed.add(h)
                        except Exception as switch_error:
                            log(f"Switch to tab failed: {str(switch_error)[:30]}", "ERROR")
                            # Force close stuck tab (will auto switch back to anchor)
                            self.force_close_tab(h)
                            continue
                        
                        # Check timeout
                        if time.time() - tab_start > max_tab_time:
                            log(f"Tab timeout, force closing...", "ERROR")
                            self.force_close_tab(h)
                            continue
                        
                        # FIX: Get URL dengan timeout protection
                        try:
                            popup_url = self.sb.driver.execute_script("return window.location.href;")
                        except:
                            log(f"Tab failed to load (no URL), closing...", "ERROR")
                            self.force_close_tab(h)
                            continue
                        
                        # Check timeout lagi
                        if time.time() - tab_start > max_tab_time:
                            log(f"Tab timeout after URL check, force closing...", "ERROR")
                            self.force_close_tab(h)
                            continue
                        
                        # Check domain
                        if not self.is_main_domain(popup_url):
                            log(f"Closing non-main popup: {popup_url[:40]}...", "ACTION")
                            self.force_close_tab(h)  # Will auto switch back to anchor
                        else:
                            log(f"Kept main domain popup: {popup_url[:40]}...", "URL")
                            
                    except Exception as e:
                        log(f"Error handling tab: {str(e)[:50]}", "ERROR")
                        self.force_close_tab(h)  # Will auto switch back to anchor
                        continue
                ...
                # CRITICAL: Always back to anchor after handling popups
                self.back_to_anchor()
                
        except Exception as e:
            log(f"Popup management error: {e}", "ERROR")
            # Reset timeout
            try:
                self.sb.driver.set_page_load_timeout(30)
            except:
                pass
            # CRITICAL: Always try to go back to anchor
            self.back_to_anchor()
            ...
            if button_info and button_info.get('found'):
                ...
                if btn_type == 'REAL':
                    ...
                    if button_info.get('selector'):
                        ...
                        selector = "//button[contains(@class, 'bg-[#1A56DB]') and contains(., 'OPEN LINK') and not(contains(@onclick, 'window.open'))]"
                    ...
                    # Build selector untuk step button
                    container_id = button_info.get('container', 'verify')
                    selector = f"//div[@id='{container_id}']//a[contains(@class, 'bg-[#1A56DB]') and contains(@onclick, 'window.open')]"
