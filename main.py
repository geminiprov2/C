...
class SafelinkBypassV2:
    ...
    def handle_popups(self):
        """Handle popup - AGGRESSIVE MODE: Tutup cepat tanpa tunggu load
        
        STRATEGY:
        1. Deteksi tab baru IMMEDIATELY
        2. JANGAN tunggu load - langsung close!
        3. NEVER close anchor tab
        ...
        try:
            ...
            log(f"[POPUP] Detected {len(new_tabs)} new tab(s) - CLOSING IMMEDIATELY", "ACTION")
            
            # AGGRESSIVE: Close ALL new tabs WITHOUT waiting for load
            closed_count = 0
            for h in new_tabs:
                # CRITICAL: Double check this is not anchor
                if h == self.anchor:
                    log(f"[POPUP] Skipping anchor tab", "INFO")
                    self.processed.add(h)
                    continue
                
                try:
                    # AGGRESSIVE: Set ultra-short timeout (1 second)
                    self.sb.driver.set_page_load_timeout(1)
                    
                    # Try switch (with timeout)
                    try:
                        self.sb.driver.switch_to.window(h)
                        self.processed.add(h)
                    except:
                        # Switch failed (tab loading/stuck) - FORCE CLOSE anyway
                        log(f"[POPUP] Tab stuck/loading - force closing...", "WARNING")
                        self.force_close_tab_aggressive(h)
                        closed_count += 1
                        continue
                    
                    # AGGRESSIVE: Get URL with ultra-short timeout
                    try:
                        popup_url = self.sb.driver.execute_script("return window.location.href;")
                    except:
                        # URL not ready (still loading) - CLOSE IT!
                        log(f"[POPUP] Tab not ready - closing...", "ACTION")
                        self.force_close_tab_aggressive(h)
                        closed_count += 1
                        continue
                    ...
                    if not self.is_main_domain(popup_url):
                        log(f"[POPUP] Closing: {popup_url[:40]}...", "ACTION")
                        self.force_close_tab_aggressive(h)
                        closed_count += 1
                    else:
                        log(f"[POPUP] Keeping main domain: {popup_url[:40]}...", "INFO")
                        
                except Exception as e:
                    log(f"[POPUP] Error handling tab - force closing: {str(e)[:30]}", "ERROR")
                    self.force_close_tab_aggressive(h)
                    closed_count += 1
            ...
            if closed_count > 0:
                log(f"[POPUP] ✓ Closed {closed_count} popup(s)", "SUCCESS")
